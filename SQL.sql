...
(
	...
	_Sex char(3) DEFAULT '男' CHECK(_Sex in('男', '女')),
	_Birth date DEFAULT '2000-01-01',
	_Nation char(6) DEFAULT '汉',
	_Payment char(10) DEFAULT '未缴费' CHECK(_Payment in('未缴费', '预缴费', '已缴费')),
	_Link char(20) DEFAULT '+86**********',
	_Teacher int FOREIGN KEY REFERENCES teacher(_Id) DEFAULT 1701000,
	_Grade char(10) DEFAULT '高一' CHECK(_Grade in('一年级', '二年级', '三年级', '四年级', '五年级', '六年级', '七年级', '八年级', '九年级', '高一' ,'高二' ,'高三')),
	...
	_ClassPerform char(3) DEFAULT '优' CHECK(_ClassPerform in('优', '良', '差')),
	_StudyPerform char(3) DEFAULT '优' CHECK(_StudyPerform in('优', '良', '差')),
...
(
	_IdStu int FOREIGN KEY REFERENCES student(_Id),
	_IdCou int FOREIGN KEY REFERENCES course(_Id),
	_Name char(40) NOT NULL,
	_Teacher int FOREIGN KEY REFERENCES teacher(_Id),
...
(
	_Id int PRIMARY KEY,
	_Type char(20) DEFAULT 'Public' CHECK (_Type in('Administrator', 'Teacher', 'student', 'public')),
